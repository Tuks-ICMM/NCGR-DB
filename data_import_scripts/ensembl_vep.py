...
f = pd.read_excel(
    "/usr/share/nginx/html/data_files/ensembl_vep.xlsx"
)
...
f = pd.read_excel(
    ...
    sheet_name="Ensembl_VEP",
)

for index, row in f.iterrows():
    a = VariantDetails.objects.filter(title=row["Variant_name"])
    b = EnsemblVep.objects.filter(
        variant=a[0],
    )
    if a.exists() and not b.exists():
        print("missing entry")
        s.variant = a[0]
        s.ensembl_canonical_hgvsc = row["Ensembl_canonical_HGVSC"]
        s.consequence_terms = row["Consequence_terms"]
        s.af_1000gp3_eur = row["AF_1000gp3_EUR"]
        s.af_1000gp3 = row["AF_1000gp3"]
        s.af_1000gp3_afr = row["AF_1000gp3_afr"]
